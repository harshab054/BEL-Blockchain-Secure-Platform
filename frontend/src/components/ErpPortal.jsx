...
function Overview({ dashboard, user }) {
  return <div className="erp-page"><div className="erp-page-heading"><div><p className="erp-kicker">Current workspace</p><h1>{dashboard.workspace.department}</h1><p>{dashboard.workspace.unit} · {dashboard.workspace.sbu} · <strong>{dashboard.workspace.access}</strong></p></div><div className="erp-role-badge">{user.accessLevel === 'STANDARD' ? dashboard.workspace.access : user.accessLevel}</div></div><section className="erp-stat-grid" data-tour="dashboard-overview">{dashboard.counts.length ? dashboard.counts.map((item) => <article key={item.module}><small>{humanize(item.module)}</small><strong>{item.count}</strong><span>Authorized records</span></article>) : <article><small>Workspace status</small><strong>Ready</strong><span>No operational records assigned yet</span></article>}</section><section className="erp-panel"><div className="erp-panel-title"><h2>My recent activity</h2><span>Personal audit scope</span></div>{dashboard.recentActivity.length ? <div className="erp-activity">{dashboard.recentActivity.map((item) => <div key={item.auditId}><b>{humanize(item.action)}</b><span>{item.targetId || 'ERP workspace'} · {formatDate(item.createdAt)}</span><em>{item.result}</em></div>)}</div> : <p className="erp-empty">No personal activity has been recorded in this demo session.</p>}</section></div>;
}

function Records({ title, records, loading }) {
  return <div className="erp-page"><div className="erp-page-heading"><div><p className="erp-kicker">Authorized module</p><h1>{title}</h1><p>Only records within the selected authorized unit, department, and SBU are displayed.</p></div></div><section className="erp-panel" data-tour="module-records">{loading ? <p className="erp-empty">Loading authorized records…</p> : records.length ? <div className="erp-table-wrap"><table className="erp-table"><thead><tr><th>Record</th><th>Description</th><th>Status</th><th>Amount</th><th>Created</th></tr></thead><tbody>{records.map((record) => <tr key={record.recordId}><td>{record.recordId}</td><td>{record.title}</td><td><span className="erp-status">{record.status}</span></td><td>{record.amount ? `₹${Number(record.amount).toLocaleString('en-IN')}` : '—'}</td><td>{formatDate(record.createdAt)}</td></tr>)}</tbody></table></div> : <p className="erp-empty">No records are authorized for this workspace.</p>}</section></div>;
}
...
export function ErpPortal() {
  ...
  const loadRecords = useCallback(async (module) => { setActiveModule(module); if (['dashboard', 'access-control', 'audit', 'organization', 'users', 'blockchain', 'profile'].includes(module)) return; setLoadingRecords(true); try { const data = await erpApi.records(session.token, module, session.workspace.departmentId); setRecords(data.records); } catch (err) { setError(err.message); setRecords([]); } finally { setLoadingRecords(false); } }, [session]);
  ...
  return <div className="erp-shell"><ProfileHeader user={user} workspace={session.workspace} onWorkspace={changeWorkspace} onLogout={logout} /><aside className="erp-sidebar"><div className="erp-sidebar-brand">BEL ERP <small>DEMO ENVIRONMENT</small></div><nav>{session.modules.map((module, index) => <button key={module.id} data-tour={`sidebar-${module.id}`} className={activeModule === module.id ? 'active' : ''} onClick={() => loadRecords(module.id)}><span>{String(index + 1).padStart(2, '0')}</span>{module.label}</button>)}</nav><div className="erp-scope"><strong>{session.workspace.access}</strong><span>{session.workspace.unit}</span><span>{session.workspace.department}</span><span>{session.workspace.sbu}</span></div></aside><main className="erp-content">{error && <p className="erp-error">{error}</p>}{activeModule === 'dashboard' && <Overview dashboard={dashboard} user={user} />}{activeModule === 'access-control' && <AccessControl user={user} bootstrap={bootstrap} token={session.token} onChanged={() => loadDashboard(session.token, session.workspace.departmentId)} />}{activeModule === 'audit' && <Audit token={session.token} />}{activeModule === 'organization' && <MasterData bootstrap={bootstrap} type="organization" />}{activeModule === 'users' && <MasterData bootstrap={bootstrap} type="users" />}{activeModule === 'blockchain' && <BlockchainRegistry token={session.token} />}{activeModule === 'profile' && <div className="erp-page"><div className="erp-page-heading"><div><p className="erp-kicker">Backend-managed profile</p><h1>My Profile</h1><p>Organizational assignments and base permissions cannot be changed from this portal.</p></div></div><section className="erp-panel erp-profile-grid" data-tour="profile-details">{Object.entries({ 'Employee ID': user.employeeId, 'Official email': user.officialEmail, Unit: user.unit, Department: user.department, SBU: user.sbu, Role: user.role, 'MFA ready': user.mfaEnabled ? 'Enabled' : 'Ready for enrollment' }).map(([label, value]) => <div key={label}><small>{label}</small><strong>{value}</strong></div>)}</section></div>}{!['dashboard', 'access-control', 'audit', 'organization', 'users', 'blockchain', 'profile'].includes(activeModule) && <Records title={activeLabel} records={records} loading={loadingRecords} />}</main><ErpGuide sessionToken={session.token} user={user} modules={session.modules} activeModule={activeModule} onNavigate={loadRecords} /></div>;
}
